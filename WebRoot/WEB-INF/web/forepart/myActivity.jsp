...
<body>
	<div class="container cf zoom">
		...
		<div class="main fl cf">
			<!-- 提示信息 -->
			<a href="<%=path%>/addActivityJsp">发表活动</a> <a
				href="<%=path %>/${actionName}?condition=datetime&club.id=${club.id}&merchant.id=${merchant.id}">时间排序</a>
			<a
				href="<%=path %>/${actionName}?condition=heat&club.id=${club.id}&merchant.id=${merchant.id}">热度排序</a>
			<s:property value="message" />
			<br />
			<s:iterator value="acs" id="ac">
				<div
					style="display: inline-block; border:1px solid;width:80%;margin:20px; overflow:hidden;">
					活动标题：<a href="activity?activity.id=${id}"><s:property
							value="tittle" /> </a><br /> 活动 封面：<br /> <img
						src="${titleImgPath}" width="100px" height="100px" /><br />
					活动预算资金：${budget}元<br /> 活动组织者：${organizer}<br /> 活动举办地点：${place}<br />
					参与人数：${participantCount}<br /> 活动状态：
					<s:property value="status.name" />
					<br /> 活动主办方：${club.name}${merchant.name}<br />
					来自：${ac.club.name}${ac.merchant.name}&nbsp; ${datetime}&nbsp;
					活动热度：${heat}<br /> 分享次数：${talking.shareDetails.shareCount} &nbsp;
