...
public class SearchAction extends BaseAction
{
	...
	public String SearchAll(){
		...
		if(sessionUser != null)
		{
			...
			merchantList = userService
					.getFocusList(Merchant.class, sessionUser);
		}
		else if(sessionMerchant != null)
		{
			clubList = merchantService
					.getFocusList(Club.class, sessionMerchant);
			merchantList = merchantService.getFocusList(Merchant.class,
					sessionMerchant);
		}
		// 用户关注商家的checkList
		...
		{
			getRequestMap()
					.put("clubMembers", clubService.clubMembersMap(club));
		}
	...
	public String SearchClub(){
		...
		if(sessionUser != null)
		{
			...
			if((clubMember = (ClubMember) getRequestMap().get(
					BaseAction.CLUB_MEMBER)) != null)
			{
				getRequestMap().put("clubMembers",
						clubService.clubMembersMap(club));
				Club club = new Club();
				club.setId(clubMember.getId().getClubId());
				List<Club> clubList2 = clubService.getFocusList(Club.class,
						club);
	...
	public String SearchMerchant(){
		...
		if((clubMember = (ClubMember) getSessionMap().get("clubMember")) != null)
		{
			Club club = new Club();
			club.setId(clubMember.getId().getClubId());
			List<Merchant> clubList2 = clubService.getFocusList(Merchant.class,
					club);
			List<Integer> clubCheckList = new ArrayList<Integer>();
