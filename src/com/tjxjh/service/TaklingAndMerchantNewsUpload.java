...
public class TaklingAndMerchantNewsUpload {
	public static boolean upload(Talking talking,MerchantNews merchantNews,File uploadImage,String UPLOAD_IMAGE_PATH,String uploadImageFileName){
		HttpServletRequest request=ServletActionContext.getRequest();
		if(uploadImage!=null){
			String urlType=FileUtil.allowfile(uploadImageFileName);
			if(urlType==null){
				System.out.println("不支持的文件，允许上传的文件格式包括：.jpg .jpeg .png .flv .f4v .mp4");
				return false;
			}
			...
			if(urlType.equals(TalkingUrlType.PICTURE.toString()))
			{
			    ...
			    if (savepath!=null&&flage==true)
				{
			    	...
			    	if(talking!=null){
			    		...
			    		merchantNews.setUrl(request.getContextPath()+savepath);
				...
				savepath = FileUtil.copyFile(uploadImage.getAbsolutePath(),imagePath);//将视频上传至服务器
			    if (savepath!=null)
				{
			    	//系统根目录为tomcat 根目录，故需要添加网站更目录
			...
			if(talking.getUrl()!=null&&!talking.getUrl().equals("")){
				...
				if(type!=null){
					...
					System.out.println("无法识别的多媒体地址，允许上传的文件格式包括：.jpg .jpeg .png,及视频分享html代码。格式如：http://www.tjxjh.com/a.jpg 或 <embed ...></embed>视频连接");
					return false;
				}
			}
		}
